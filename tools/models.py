...
def iou_loss(roi_gt, byte_mask, roi_pred, box_loss_value):
  ...
  area_gt_l = (d1_gt[mask3] + d2_gt[mask3]) * (d3_gt)           # 顺序为：上，下，左，右
  ...
  AABB_l = - torch.log((area_intersect_l + 1.0)/(area_union_l + 1.0))       # 左边部分的IOUloss
...
class ModelResNetSep2(nn.Module):
  ...
  def __init__(self, attention = False, multi_scale = True, nclass = 7500):
    ...
    self.conv11 = Conv2d(256, nclass, (1, 1), padding=(0,0))
  ...
  def forward(self, x):
    ...
    segm_pred = torch.sigmoid(self.act(x))              # x为1/4
    rbox = torch.sigmoid(self.rbox(x)) * 128            # 到边框的距离
    angle = torch.sigmoid(self.angle(x)) * 2 - 1        # 为啥会输出2个值呢
    angle_den = torch.sqrt(angle[:, 0, :, :] * angle[:, 0, :, :] + angle[:, 1, :, :] * angle[:, 1, :, :]).unsqueeze(1)
    angle_den = angle_den.expand_as(angle)
    angle = angle / angle_den
    
    return [segm_pred, segm_pred2], [rbox, rbox2], [angle, angle2], [x, focr]        # 两个不同尺度的输出
  ...
  def loss(self, segm_preds, segm_gt, iou_mask, angle_preds, angle_gt, roi_pred, roi_gt):
    ...
    if self.multi_scale:                  # 多尺度，在1/4和1/8上都有输出
    ...
    byte_mask = torch.gt(segm_gt, 0.5)    # 将大于0.5的判定为目标区域
    
    if byte_mask.sum() > 0:               # 是目标的区域
...
class OwnModel(ModelResNetSep2):
  def __init__(self, attention = False, multi_scale=True, nclass=7500):
    super(OwnModel, self).__init__(attention=attention, multi_scale=multi_scale, nclass=nclass)
