...
from dataset import custom_dataset, fots_collotfn, ocr_dataset
...
def train(train_img_path, train_gt_path, pths_path, batch_size, lr, num_workers, epoch_iter, interval, ocr_branch):
	...
	ocrset = ocr_dataset(train_img_path, train_gt_path)
	ocrgen = data.DataLoader(ocrset, batch_size=batch_size, \
                            shuffle=True, num_workers=num_workers, drop_last=True, collate_fn=fots_collotfn())
	...
	for epoch in range(epoch_iter):	
		...
		for i, (ocr_img, text_polys, texts) in enumerate(ocrgen):
			try:
