...
def draw_box_points(img, points, color = (0, 255, 0), thickness = 1):
	...
	cv2.circle(img, (int(points[0][0]), int(points[0][1])), 10, (0, 255, 0), -1)             # 第一个点绿色
	cv2.circle(img, (int(points[1][0]), int(points[1][1])), 10, (0, 0, 255), -1)             # 第二个点红色
	cv2.circle(img, (int(points[2][0]), int(points[2][1])), 10, (255, 0, 0), -1)             # 第三个点蓝色
...
def resize_32(pil_img, vertices):
	vertices = vertices.astype(np.float64)
	im = np.asarray(pil_img)
	image_size = [im.shape[0] // 32 * 32, im.shape[1] // 32 * 32]
	resize_h = int(image_size[0])
	resize_w = int(image_size[1])
	scaled = cv2.resize(im, dsize=(resize_w, resize_h))
	scaled = Image.fromarray(scaled)

	scalex = resize_w / im.shape[1]
	scaley = resize_h / im.shape[0]
	if len(vertices) > 0:
		vertices[:, [0, 2, 4, 6]] *= scalex
		vertices[:, [1, 3, 5, 7]] *= scaley
...
class custom_dataset(data.Dataset):
	...
	def __getitem__(self, index):
		with open(self.gt_files[index], 'r') as f:
			lines = f.readlines()
		vertices, labels, texts = extract_vertices(lines)
		
		img = Image.open(self.img_files[index])
		img, vertices = adjust_height(img, vertices)

		ocr_img, ocr_vertices = resize_32(img, vertices)

		img, vertices = rotate_img(img, vertices)
		# #for debug show
		# im = np.array(ocr_img)
		# for box in ocr_vertices:
		# 	pts = box[0:8]
		# 	pts = pts.reshape(4, -1)
		...
		return transform(img), score_map, geo_map, ignored_map, ocr_img, ocr_vertices, texts
...
class ocr_dataset(data.Dataset):
	...
	def __getitem__(self, index):
		...
		ocr_img, ocr_vertices = resize_32(img, vertices)
		return ocr_img, ocr_vertices, texts


class fots_collotfn(object):
	def __init__(self):
	...
	def __call__(self, batch, **kwargs):
		east_image, score_map, geo_map, ignored_map, ocr_image, vertices, texts = zip(*batch)
		# ocr_image, vertices, texts = zip(*batch)
		maxw = max(p.width for p in ocr_image)
		maxh = max(p.height for p in ocr_image)

		paded_images = []
		paded_gt = []
		for (p, text_poly) in zip(ocr_image, vertices):
			top = maxh - p.height
			left = maxw - p.width
			bottom = 0
			right = 0
			p = np.asarray(p)
			im = cv2.copyMakeBorder(p, top, bottom, left, right, cv2.BORDER_CONSTANT)  # 填充图像
			if len(text_poly) > 0:
				text_poly[:, [0, 2, 4, 6]] += left
				text_poly[:, [1, 3, 5, 7]] += top
			...
			paded_images.append(im)
			paded_gt.append(text_poly)

		# return torch.stack(paded_images, 0), paded_gt, texts
		return torch.stack(east_image, 0), torch.stack(score_map, 0), torch.stack(geo_map, 0), torch.stack(ignored_map, 0), \
					 torch.stack(paded_images, 0), paded_gt, texts